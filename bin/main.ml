...
| (x, Game_state.Illegal, p) -> (print_endline "Illegal move, please enter new move."; (x, p))

let update_state (state : Game_state.game_state) new_board taken = match taken with
| (Game_state.Empty, _) -> {state with board = new_board}
| x -> match x with
      | (p, Game_state.White) -> {state with board = new_board; white_taken = p :: state.white_taken}
      | (p, Game_state.Black) -> {state with board = new_board; black_taken = p :: state.black_taken}
